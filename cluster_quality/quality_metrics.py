...
for idx1, i in enumerate(cluster_ids):
    ...
    if do_parallel:
        ...
        scores = Parallel(n_jobs=-1, verbose=10)(delayed(score_inner_loop)(i, cluster_ids) for i in cluster_ids)
    else:
...
def calculate_drift_metrics(spike_times,
    ...
    if do_parallel:
        from joblib import Parallel, delayed
        meas = Parallel(n_jobs=-1, verbose=10)(delayed(calc_one_cluster)(cluster_id)
